...
export default function GameBlackjack({ profile, setProfile, goBack, showToast }) {
    ...
    }, [deck, playerHand, dealerHand, betAmount, gameState, gameResult, earnedCoins, isLoaded, storageKey]);
        ...
        <div className="pb-10 animate-in fade-in zoom-in-95 max-w-2xl mx-auto relative select-none">
            ...
            <div className="bg-[#0b291d] border-4 border-[#12422e] rounded-3xl p-4 sm:p-6 shadow-2xl relative overflow-hidden min-h-[350px] sm:min-h-[400px] flex flex-col justify-between">
                    ...
                    <div className="mb-2 sm:mb-4">
                        <div className="flex justify-between items-end mb-2">
                            ...
                            <span className="bg-black/50 px-3 py-1 rounded-full font-black text-white/90 text-sm">
                                {gameState === "playing" ? getCardValue(dealerHand[0].rank) : getHandScore(dealerHand)}
                            </span>
                        </div>
                        ...
                        <div className="flex justify-center -space-x-10 sm:-space-x-14">
                            {dealerHand.map((card, idx) => {
                                ...
                                return (
                                    <div key={idx} className={`relative transform transition-all duration-300 ${isHiddenCard ? '' : 'hover:-translate-y-2'}`} style={{ zIndex: idx, animation: `flyInTop 0.5s ease-out ${idx < 2 ? 0.15 + idx * 0.3 : 0}s backwards`, perspective: '1000px' }}>
                                        {showBack ? (
                                            <div className={`w-20 sm:w-24 h-full relative transition-transform duration-700 preserve-3d`} style={{ transform: isHiddenCard ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
                                                <img
                                                    src={card.image}
                                                    alt="card front"
                ...
                <div className="flex-1 flex items-center justify-center my-2 min-h-[40px]">
                    {gameState === "game_over" && (
                        <div className={`px-6 py-3 rounded-2xl font-black text-xl sm:text-2xl uppercase tracking-widest shadow-2xl animate-in zoom-in ${gameResult === "win" ? "bg-green-500 text-green-950" :
                            gameResult === "blackjack" ? "bg-fuchsia-500 text-white" :
                                gameResult === "push" ? "bg-blue-500 text-white" :
                                    "bg-red-600 text-white"
                            }`}>
                            {gameResult === "win" ? "Перемога!" : gameResult === "blackjack" ? "Блекджек!" : gameResult === "push" ? "Нічия" : "Поразка"}
                        </div>
                    )}
                ...
                {gameState !== "betting" && (
                    <div className="mt-2 sm:mt-4">
                        <div className="flex justify-between items-end mb-2">
                            <h3 className="text-white/80 font-bold uppercase tracking-widest text-sm">Ваша Рука</h3>
                            <span className="bg-black/50 px-3 py-1 rounded-full font-black text-white/90 text-sm">
                        ...
                        <div className="flex justify-center -space-x-10 sm:-space-x-14">
                            {playerHand.map((card, idx) => (
                                <div key={idx} className="relative transform transition-all duration-300 hover:-translate-y-4" style={{ zIndex: idx, animation: `flyInBottom 0.5s ease-out ${idx < 2 ? idx * 0.3 : 0}s backwards` }}>
                                    <img
                                        src={card.image}
                                        alt="card"
                                        className="w-20 sm:w-24 h-auto rounded-xl shadow-xl border border-white/10"
                ...
                {gameState === "betting" && (
                    <div className="flex-1 flex flex-col items-center justify-center">
                        ...
                        <h2 className="text-3xl font-black text-white mb-8 tracking-wider uppercase">Зробіть ставку</h2>

                        <div className="flex items-center gap-4 bg-black/40 p-3 rounded-2xl mb-8 border border-white/5">
                            <button onClick={() => adjustBet(-100)} className="w-12 h-12 bg-red-500/20 text-red-400 rounded-xl flex items-center justify-center hover:bg-red-500 hover:text-white transition-colors"><Minus size={20} /></button>
                            <input
                                type="number"
                            ...
                            />
                            <button onClick={() => adjustBet(100)} className="w-12 h-12 bg-green-500/20 text-green-400 rounded-xl flex items-center justify-center hover:bg-green-500 hover:text-white transition-colors"><Plus size={20} /></button>
                        </div>

                        <div className="flex flex-wrap justify-center gap-3 mb-8 w-full max-w-[300px]">
                            <button onClick={() => setBetAmount(10)} className="flex-1 bg-neutral-800 hover:bg-neutral-700 text-white font-bold py-2 px-3 rounded-lg text-sm transition-colors border border-white/10">Мін.</button>
                            <button onClick={() => adjustBet(500)} className="flex-1 bg-neutral-800 hover:bg-neutral-700 text-white font-bold py-2 px-3 rounded-lg text-sm transition-colors border border-white/10">+500</button>
                            <button onClick={setMaxBet} className="flex-1 bg-orange-600/20 hover:bg-orange-600 text-orange-400 hover:text-white font-bold py-2 px-3 rounded-lg text-sm transition-colors border border-orange-500/30">Макс</button>
                        </div>
                        ...
                        >
                            {isProcessing ? <Loader2 size={24} className="animate-spin" /> : <Play size={24} fill="currentColor" />}
                            Грати ({betAmount})
                        </button>
                    </div>
                )}
