            ...
            <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar justify-between">
              {[...Array(5)].map((_, i) => {
                ...
                return card ? (
                  ...
                  >
                    <div
                      className={`w-full h-full rounded-xl overflow-hidden border-2 bg-neutral-900 group-hover:-translate-y-1 transition-transform border-indigo-500 shadow-[0_0_15px_rgba(99,102,241,0.3)] transform-gpu will-change-transform`}
                    >
                  ...
                  {ownedGameCards.map((card) => {
                    ...
                    return (
                      <div
                        key={card.uniqueInstanceId}
                        onClick={() => handleToggleCard(card)}
                        className={`relative aspect-[2/3] rounded-lg border-2 overflow-hidden bg-neutral-900 cursor-pointer hover:-translate-y-1 hover:border-indigo-500 transition-all transform-gpu will-change-transform ${getCardStyle(card.rarity).border}`}
                        title={card.name}
                      >
