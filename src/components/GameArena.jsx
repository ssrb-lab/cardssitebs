...
export default function GameArena({ profile, cardsCatalog, goBack, showToast }) {
    ...
    const handleCapturePoint = async (pointId, point, e) => {
        ...
        if (deck.length !== 5) {
            return showToast('Для захоплення точки необхідно обрати рівно 5 карт зі свого інвентаря.', 'error');
        }
                            ...
                            <div className="bg-neutral-900 border border-neutral-700 rounded-2xl p-6 w-full max-w-sm shadow-2xl flex flex-col gap-4">
                                ...
                                <div className="flex gap-4">
                                    <div className="flex-1">
                                        <label className="text-xs uppercase font-bold text-neutral-400 mb-1 block">Квиток (Монети)</label>
                                        <input
                                            type="number"
                                            value={adminPointData.entryFee}
                                            onChange={(e) => setAdminPointData({ ...adminPointData, entryFee: e.target.value })}
                                            className="w-full bg-neutral-950 border border-neutral-800 rounded-xl p-3 text-white focus:outline-none focus:border-indigo-500 transition-colors"
                                        />
                                    </div>
                                    <div className="flex-1">
                                        <label className="text-xs uppercase font-bold text-neutral-400 mb-1 block">КД (Хв)</label>
                                        <input
                                            type="number"
                                            value={adminPointData.cooldownMinutes}
                                            onChange={(e) => setAdminPointData({ ...adminPointData, cooldownMinutes: e.target.value })}
                                            className="w-full bg-neutral-950 border border-neutral-800 rounded-xl p-3 text-white focus:outline-none focus:border-indigo-500 transition-colors"
                                        />
                                    </div>
                                    <div className="flex-1">
                                        <label className="text-xs uppercase font-bold text-neutral-400 mb-1 flex items-center gap-1"><Gem size={10} className="text-fuchsia-500" /> Фарм/год</label>
                                        <input
                                            type="number"
                                            value={adminPointData.crystalRatePerHour || 0}
                                            onChange={(e) => setAdminPointData({ ...adminPointData, crystalRatePerHour: e.target.value })}
                                            className="w-full bg-neutral-950 border border-neutral-800 rounded-xl p-3 text-white focus:outline-none focus:border-indigo-500 transition-colors"
                                        />
                                    </div>
                                </div>
                        ...
                        >
                            ...
                            {!isLoadingPoints && points.map((point) => {
                                ...
                                return (
                                    ...
                                    `} style={{ borderColor: point.ownerId === profile?.uid ? '#22c55e' : (point.color || '#4f46e5') }}>
                                            <PointIcon size={16} style={{ color: point.ownerId === profile?.uid ? '#22c55e' : (point.color || '#4f46e5') }} />
                                        </div>
                                    </div>
                                );
                            })}
                    ...
                    {selectedPoint && (
                        <div className="absolute top-0 right-0 w-80 h-full max-h-full overflow-y-auto bg-neutral-900/95 backdrop-blur-md border-l border-neutral-800 shadow-[-10px_0_30px_rgba(0,0,0,0.5)] z-40 flex flex-col p-4 animate-in slide-in-from-right duration-300">
                            ...
                            <div className="flex justify-between items-start mb-4">
                                <h3 className="text-xl font-bold uppercase text-white truncate pr-2">{selectedPoint.name}</h3>
                                <button onClick={() => setSelectedPoint(null)} className="p-1 rounded bg-neutral-800 text-neutral-400 hover:text-white">
                                    <X size={18} />
                                </button>
                            </div>
                                ...
                                {selectedPoint.ownerId ? (
                                    <>
                                        <div className="flex items-center justify-between text-sm">
                                            ...
                                            <span className={`font-bold ${selectedPoint.ownerId === profile?.uid ? 'text-green-400' : 'text-red-400'}`}>
                                        ...
                                        {(() => {
                                            ...
                                            return (
                                                <div className="flex items-center justify-between text-sm">
                                                    ...
                                                    {isProtected ? (
                                                        <span className="text-yellow-400 font-bold">{m}:{s}</span>
                                                    ) : (
                            ...
                            {selectedPoint.defendingCards && selectedPoint.defendingCards.length > 0 && (
                                <div className="mb-4">
                                    <h4 className="text-xs uppercase font-bold text-neutral-500 mb-2 border-b border-neutral-800 pb-1">Захисники</h4>
                                ...
                                {selectedPoint.ownerId ? (
                                    selectedPoint.ownerId !== profile?.uid && (
                                        ...
                                        >
                                            ...
                                            {deck.length < 5 && <span className="text-[9px] font-normal opacity-70 mt-1">Оберіть ще {5 - deck.length} карт</span>}
                                        </button>
                                    )
                                ) : (
                                    <button
