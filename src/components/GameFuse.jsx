...
export default function GameFuse({ profile, setProfile, goBack, showToast }) {
    ...
    const generateLevel = () => {
        ...
        const newBoard = Array(GRID_SIZE).fill({ isDamaged: false, isFound: false, memoryFlipped: false });
    ...
    const renderFuse = (isDamaged, isFound, memoryFlipped) => {
        ...
        const showHealthy = phase === 'memorize' ? !isDamaged : (!isDamaged || (!isDamaged && (isFound || memoryFlipped)) || (isDamaged && !isFound && !memoryFlipped));
        ...
        if (showDamaged) {
            return (
                <div className={`w-[70%] h-3 bg-orange-100 relative rounded-sm rotate-45 flex items-center shadow-sm overflow-hidden opacity-90 transition-transform duration-500`}>
                    {/* Left Cap Rusty */}
        ...
        <div className="pb-10 animate-in fade-in zoom-in-95 max-w-4xl mx-auto relative select-none">
            ...
            <div className="flex flex-col md:flex-row gap-8 items-center md:items-start justify-center">
                    ...
                    <div className="flex justify-between items-center mb-6 bg-neutral-900 border border-neutral-800 p-4 rounded-2xl">
                        ...
                        <div className="flex gap-1 text-red-500">
                            {Array.from({ length: 3 }).map((_, i) => (
                                <Heart key={i} size={24} fill={i < lives ? "currentColor" : "transparent"} className={i >= lives ? "text-neutral-700" : ""} />
                            ))}
                        </div>
                    </div>
                ...
                <div className="flex-1 w-full max-w-sm flex flex-col justify-center text-center md:text-left mt-4 md:mt-20 px-4">
                    <h2 className="text-3xl sm:text-4xl font-black text-white leading-tight uppercase tracking-wide mb-4 flex items-center justify-center md:justify-start gap-3">
                        ...
                        Відмітьте всі<br />пошкоджені<br />запобіжники
                    </h2>
                    <p className="text-neutral-400 text-sm font-medium mb-6">Спочатку запам'ятайте їх розташування. Коли вони сховаються - натискайте по пам'яті. За кожну помилку ви втрачаєте 1 життя.</p>

                    <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4">
                        <div className="text-xs text-neutral-500 uppercase font-bold mb-2">Рівень навички: {progressInfo.level}</div>
                        ...
                        {progressInfo.max === "MAX" ? (
                            ...
                            <div>
                                <div className="flex justify-between text-xs text-neutral-400 mb-1 font-bold">
                                    ...
                                    <span>{progressInfo.current} / {progressInfo.max} плат</span>
                                </div>
                                ...
                                <div className="text-center text-blue-400 text-xs mt-1 font-bold">{progressInfo.percentage}%</div>
                            </div>
                        )}
                    </div>
                </div>
            ...
            {gameOver && (
                <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center rounded-3xl animate-in fade-in p-6 text-center">
                    ...
                    <p className="text-neutral-300 mb-6">Відремонтовано плат: <span className="text-white font-bold">{score}</span></p>
                    ...
                    <button onClick={claimReward} disabled={isProcessing} className="bg-yellow-500 hover:bg-yellow-400 text-yellow-950 font-black py-4 px-8 rounded-2xl flex items-center gap-2 shadow-[0_0_20px_rgba(234,179,8,0.5)]">
                        ...
                        Забрати {score * (progressInfo.level === 1 ? 75 : progressInfo.level === 2 ? 150 : progressInfo.level === 3 ? 200 : progressInfo.level === 4 ? 375 : 350)} монет
                    </button>
