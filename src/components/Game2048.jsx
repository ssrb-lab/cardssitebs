...
import { claim2048RewardRequest, getToken } from "../config/api";
...
export default function Game2048({ setProfile, goBack, showToast }) {
  ...
  useEffect(() => {
    ...
    if (saved) {
        try {
            const { board: savedBoard, score: savedScore, gameOver: savedGameOver, nextId } = JSON.parse(saved);
  ...
  useEffect(() => {
      if (isInitialized) {
          localStorage.setItem('2048_state', JSON.stringify({ board, score, gameOver, nextId: uidRef.current }));
      }
  }, [board, score, gameOver, isInitialized]);
  ...
  const move = useCallback((direction) => {
    ...
    if (oldStr !== newStr) {
      ...
      if (!movesLeft) setGameOver(true);
    }
  }, [board, score, gameOver, isProcessing]);
  ...
  return (
    <div className="pb-10 animate-in fade-in zoom-in-95 max-w-md mx-auto relative select-none">
      ...
      {gameOver && (
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center rounded-3xl animate-in fade-in">
              <Trophy size={60} className="text-yellow-500 mb-4" />
              <h2 className="text-3xl font-black text-white mb-2 uppercase">Гру закінчено!</h2>
              ...
              <button onClick={restartGame} className="mt-4 text-neutral-400 hover:text-white font-bold underline">
                  Грати заново (без отримання нагороди)
              </button>
          </div>
      )}
    </div>
