  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setIsInitialized(true);
        if (!savedGameOver) start2048GameRequest(getToken()).catch(() => {});
        return;
      } catch {
        // ignore
      }
  ...
  useEffect(() => {
    if (isInitialized) {
      localStorage.setItem(
        '2048_state',
        JSON.stringify({ board, score, gameOver, gameWon, nextId: uidRef.current })
      );
    }
  ...
  const move = useCallback(
    (direction) => {
      ...
      let newBoard = board.map((row) =>
        row.map((tile) => (tile ? { ...tile, isNew: false, merged: false } : null))
      );
      let newScore = score;
        ...
        for (let i = 0; i < GRID_SIZE; i++) {
          for (let j = 0; j < GRID_SIZE; j++) {
            ...
            if (
              ...
              newBoard[i][j] !== null &&
              newBoard[i][j + 1] !== null &&
              newBoard[i][j].val === newBoard[i][j + 1].val
            )
              movesLeft = true;
            if (
              i < GRID_SIZE - 1 &&
              newBoard[i][j] !== null &&
              newBoard[i + 1][j] !== null &&
              newBoard[i][j].val === newBoard[i + 1][j].val
            )
              movesLeft = true;
          }
        }
  ...
  const claimReward = async () => {
    ...
    try {
      ...
      if (data.earned > 0) {
        ...
        showToast(data.message || 'Ви не отримали монет (ліміт вичерпано).', 'success');
      }
      localStorage.removeItem('2048_state');
      goBack();
    } catch (e) {
    ...
    <div className="pb-1 sm:pb-10 pt-1 sm:pt-0 animate-in fade-in zoom-in-95 w-full h-full overflow-hidden max-w-md mx-auto flex flex-col items-center justify-start sm:justify-center relative select-none">
      ...
      >
        <div className="relative aspect-square" style={{ width: 'min(50dvh, 90vw, 400px)' }}>
          ...
          {activeTiles.map((t) => (
            ...
            >
              <div
                className={`w-full h-full flex items-center justify-center rounded-xl font-black text-2xl sm:text-3xl shadow-sm ${t.isNew ? 'animate-in zoom-in-50 duration-200' : ''} ${t.merged ? 'animate-in zoom-in-110 duration-150' : ''} ${getTileColor(t.val)}`}
              >
                {t.val}
              </div>
            </div>
      ...
      {gameOver && (
        <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center rounded-3xl animate-in fade-in p-6 text-center">
          ...
          <h2
            className={`text-3xl font-black mb-2 uppercase ${gameWon ? 'text-fuchsia-400' : 'text-white'}`}
          >
