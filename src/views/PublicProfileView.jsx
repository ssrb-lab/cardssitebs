    ...
    const fetchPlayerData = async () => {
      try {
        ...
        if (profileData.mainShowcaseId && profileData.showcases) {
          const showcaseFound = profileData.showcases.find(
            (s) => s.id === profileData.mainShowcaseId
          );
          if (showcaseFound) {
            const showcaseCards = [];
            const tempInv = JSON.parse(JSON.stringify(inv));
            const cardIdsArray =
              typeof showcaseFound.cardIds === 'string'
                ? JSON.parse(showcaseFound.cardIds)
                : showcaseFound.cardIds;
  ...
  if (loading)
    return (
      <div className="text-center py-20">
        <Loader2 className="animate-spin mx-auto w-12 h-12 text-yellow-500 mb-4" /> Шукаємо
        гравця...
      </div>
    );
  if (!playerInfo)
    return (
      <div className="text-center py-20 text-neutral-500">
        <Ban className="mx-auto w-12 h-12 mb-4" />
        Гравця не знайдено.
      </div>
    );
  ...
  return (
    <div className="pb-10 animate-in fade-in slide-in-from-right-8 duration-500">
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-8 text-center relative overflow-hidden mb-8 shadow-xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 sm:gap-4 mb-8">
          <div className="bg-neutral-900 border border-neutral-800 p-4 rounded-2xl flex flex-col items-center justify-center text-center shadow-md">
            ...
            <span className="text-[10px] sm:text-xs text-neutral-500 font-bold uppercase mt-1">
              Баланс
            </span>
          </div>
          <div className="bg-neutral-900 border border-neutral-800 p-4 rounded-2xl flex flex-col items-center justify-center text-center shadow-md">
            ...
            {mainShowcase.validCards.length === 0 ? (
              ...
              mainShowcase.validCards.map((c, index) => {
                ...
                return (
                  <div
                    key={index}
                    onClick={() => setViewingCard({ card: c, amount: 1 })}
                    className="relative group cursor-pointer animate-in zoom-in-95"
                  >
                    <div
                      className={`w-24 sm:w-32 aspect-[2/3] rounded-xl border-2 overflow-hidden bg-neutral-950 ${style.border} ${effectClass} transition-transform group-hover:-translate-y-2`}
                    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          <select
            ...
            className="bg-neutral-900 border border-neutral-700 text-sm font-medium rounded-xl px-4 py-3 focus:outline-none text-white flex-1 min-w-[140px]"
          >
            {categories.map((c) => (
          ...
          <select
            ...
            className="bg-neutral-900 border border-purple-900/50 text-sm font-bold rounded-xl px-4 py-3 focus:outline-none text-purple-400 flex-1 min-w-[140px]"
          >
          ...
          return (
            ...
            >
              <div
                ...
                className={`relative w-full aspect-[2/3] rounded-xl border-2 overflow-hidden bg-neutral-900 mb-2 transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-[0_10px_20px_rgba(0,0,0,0.5)] ${style.border} ${effectClass}`}
              >
      ...
      {filteredInventory.length === 0 && (
        <p className="text-center text-neutral-500 py-10">Картки не знайдено.</p>
      )}
    </div>
  );
}
