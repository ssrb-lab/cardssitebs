...
function PlayerAvatar({ profile, className = '', iconSize = 24 }) {
  ...
  const bgClass = profile?.isSuperAdmin
  ...
  return (
    <div
      className={`flex items-center justify-center font-bold border-2 shadow-sm shrink-0 ${bgClass} ${className}`}
    >
      {profile?.isSuperAdmin ? (
        <Crown size={iconSize} />
      ) : profile?.isAdmin ? (
      ...
      (snap) => {
        if (snap.exists()) {
          ...
          setPremiumDailyRewards(
            data.premiumDailyRewards || [2000, 4000, 6000, 8000, 10000, 12000, 15000]
          );
          setPremiumPrice(data.premiumPrice !== undefined ? data.premiumPrice : 10000);
          setPremiumDurationDays(
            data.premiumDurationDays !== undefined ? data.premiumDurationDays : 30
          );
      ...
      if (docSnap.exists()) {
        ...
        if (needsUpdate) {
          updateDoc(profileRef, {
            isBanned: pData.isBanned,
            banReason: pData.banReason,
            banUntil: pData.banUntil,
            isPremium: pData.isPremium,
            premiumUntil: pData.premiumUntil,
          });
        }
  ...
  useEffect(() => {
    if (user && profile && dbInventory) {
      if (profile.uniqueCardsCount !== dbInventory.length) {
        const profileRef = doc(db, 'artifacts', GAME_ID, 'public', 'data', 'profiles', user.uid);
        updateDoc(profileRef, { uniqueCardsCount: dbInventory.length }).catch((e) =>
    ...
    const trackIp = async () => {
      try {
        ...
        if (profile.lastIp !== currentIp) {
          ...
          const q = query(
            collection(db, 'artifacts', GAME_ID, 'public', 'data', 'profiles'),
            where('lastIp', '==', currentIp)
          );
          ...
          if (altAccounts.length > 0) {
            ...
            addSystemLog(
              '⚠️ Мультиакаунт',
              `Гравець ${profile.nickname} зайшов з IP (${currentIp}), який використовують: ${altAccounts.join(', ')}`
            );
          } else if (!profile.lastIp) {
            // Перший раз зайшов
            addSystemLog(
              'ℹ️ Новий IP',
              `Гравець ${profile.nickname} вперше зафіксував IP: ${currentIp}`
            );
          } else {
            // Змінив мережу
            addSystemLog(
              'ℹ️ Зміна мережі',
              `Гравець ${profile.nickname} змінив IP на: ${currentIp}`
            );
          }
  ...
  const handleAuthSubmit = async (e) => {
    ...
    try {
      if (authMode === 'register') {
        ...
        const allProfilesSnap = await getDocs(
          collection(db, 'artifacts', GAME_ID, 'public', 'data', 'profiles')
        );
        let exists = false;
    ...
    try {
      ...
      showToast(
        listing.sellerUid === user.uid
          ? 'Ваш лот знято з продажу.'
          : 'Лот гравця примусово видалено.',
        'success'
      );
      if (listing.sellerUid !== user.uid) {
        addSystemLog(
          'Адмін',
          `Адмін ${profile.nickname} примусово видалив лот гравця ${listing.sellerNickname}`
        );
      }
    } catch (e) {
      console.error(e);
        ...
        <div className="bg-neutral-900 border border-neutral-800 p-8 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.8)] max-w-md w-full relative z-10 animate-in slide-in-from-bottom-8 duration-500">
          ...
          <div className="mt-4">
            <button
              ...
              type="button"
              className="w-full bg-white text-neutral-900 hover:bg-gray-100 font-bold py-4 px-4 rounded-xl transition-all shadow-lg flex items-center justify-center gap-2"
            >
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans pb-24 relative overflow-x-hidden">
      ...
      <header className="bg-neutral-900 border-b border-neutral-800 sticky top-0 z-50 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-3 flex justify-between items-center">
          {/* Логотип Бренду */}
          <div
            className="flex items-center gap-2 sm:gap-3 text-white font-black text-lg tracking-wider cursor-pointer group"
          ...
          >
            ...
            <span className="hidden sm:block bg-clip-text text-transparent bg-gradient-to-r from-white to-neutral-400 group-hover:to-white transition-colors">
              Card Game
            </span>
          </div>
          ...
          <div className="flex items-center gap-3 sm:gap-6">
            ...
            >
              ...
              <div className="hidden md:block text-left">
                <div className="font-bold text-sm leading-tight text-white flex items-center gap-1">
                  {profile?.nickname}
                  <span
                    className="bg-red-900/50 text-red-400 text-[10px] px-1.5 py-0.5 rounded-md border border-red-800 flex items-center gap-0.5 ml-1"
                    title="Рівень Фарму"
                  >
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <div className="bg-neutral-950 px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl border border-neutral-800 shadow-inner flex gap-2 items-center">
                ...
                <span className="text-yellow-500 font-black text-base sm:text-lg">
                  {profile?.coins}
                </span>
              </div>
              <button
              ...
              >
                ...
                <span className="hidden sm:block text-fuchsia-400 font-bold text-sm tracking-wide">
                  Преміум
                </span>
              </button>
            </div>
      ...
      {toastMsg.text && (
        <div
          className={`fixed top-20 left-1/2 -translate-x-1/2 px-6 py-3 rounded-full flex items-center gap-2 shadow-lg z-50 animate-bounce text-white font-medium whitespace-nowrap ${
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-8 text-center relative overflow-hidden mb-8 shadow-xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 relative z-10 max-w-2xl mx-auto">
          ...
          <div className="bg-neutral-950 border border-neutral-800 rounded-2xl p-4 flex flex-col items-center">
            ...
            <span className="text-[10px] text-neutral-500 font-bold uppercase mt-1">
              Витрачено <Coins size={8} className="inline" />
            </span>
          </div>
          <div className="bg-neutral-950 border border-neutral-800 rounded-2xl p-4 flex flex-col items-center">
            <Zap className="text-green-500 mb-2 w-6 h-6" />
            <span className="text-xl font-black text-white">
              {profile?.coinsEarnedFromPacks || 0}
            </span>
            <span className="text-[10px] text-neutral-500 font-bold uppercase mt-1">
              Виграно <Coins size={8} className="inline" />
            </span>
          </div>
        </div>
      </div>
            ...
            {validShowcaseCards.map((card, idx) => {
              ...
              return (
                <div
                  ...
                  className="relative group cursor-pointer animate-in zoom-in-95 hover:-translate-y-2 transition-transform"
                >
                  <div
                    className={`w-28 sm:w-36 aspect-[2/3] rounded-xl border-2 overflow-hidden bg-neutral-950 shadow-lg ${style.border} ${effectClass}`}
                  >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 relative overflow-hidden group">
          ...
          <p className="text-sm text-neutral-400 mb-4 relative z-10">
            Отримуйте монети кожного дня! {isPremiumActive ? 'Преміум дає більше нагород.' : ''}
          </p>
          <button
            onClick={claimDaily}
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 relative overflow-hidden group">
          ...
          <p className="text-sm text-neutral-400 mb-4 relative z-10">
            Введіть код для отримання бонусів.
          </p>
          <form onSubmit={redeemPromo} className="flex gap-2 relative z-10">
            ...
            <button
              ...
              className="bg-purple-600 hover:bg-purple-500 disabled:bg-neutral-800 text-white font-bold px-6 py-3 rounded-xl transition-colors"
            >
              Ок
            </button>
          </form>
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 relative overflow-hidden group md:col-span-2">
          ...
          <div className="flex flex-col sm:flex-row gap-6 mt-4">
            <div className="flex-1">
              ...
              <form onSubmit={handleAvatarUpdate} className="flex gap-2 relative z-10">
                ...
                <button
                  ...
                  className="bg-blue-600 hover:bg-blue-500 disabled:bg-neutral-800 text-white font-bold px-4 py-3 rounded-xl transition-colors text-sm"
                >
                  Зберегти
                </button>
              </form>
            ...
            <div className="flex-1 flex items-end">
              <button
                onClick={handleLogout}
                className="w-full bg-red-900/40 hover:bg-red-900 text-red-400 hover:text-white font-bold py-3 px-6 rounded-xl transition-colors flex justify-center items-center gap-2 border border-red-900/50"
              >
                <LogOut size={18} /> Вийти з акаунту
              </button>
            </div>
...
function PremiumShopView({
  ...
  const buyPremium = async () => {
    if (isProcessing) return;
    if (profile.coins < premiumPrice)
      return showToast('Недостатньо монет для покупки Преміум-акаунту!');
    ...
    try {
      ...
      addSystemLog(
        'Магазин',
        `Гравець ${profile.nickname} купив Преміум за ${premiumPrice} монет.`
      );
    } catch (e) {
      console.error(e);
  ...
  const buyItem = async (item) => {
    if (isProcessing) return;
    if (!isPremiumActive)
      return showToast('Цей товар доступний лише для власників Преміум-акаунту!');
    ...
    try {
      ...
      addSystemLog(
        'Магазин',
        `Гравець ${profile.nickname} купив ексклюзив ${item.name} за ${item.price} монет.`
      );
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="pb-10 animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="bg-gradient-to-br from-neutral-900 to-blue-950/30 border-2 border-blue-500/30 rounded-3xl p-6 sm:p-10 text-center max-w-2xl mx-auto shadow-[0_0_40px_rgba(59,130,246,0.15)] mb-12 relative overflow-hidden">
        <Edit2
          size={48}
          className="mx-auto text-blue-400 mb-4 drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]"
        />
        ...
        >
          <input
            ...
            required
            className="flex-1 bg-neutral-950 border border-neutral-700 rounded-xl px-4 py-3 text-white focus:border-blue-500 outline-none"
          />
          <button
            type="submit"
            disabled={isProcessing}
            className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-6 rounded-xl transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
          >
            Купити{' '}
              ...
              return (
                ...
                >
                  ...
                  <h3 className="text-xl font-bold text-white mb-2 text-center w-full">
                    {item.name || (cDef ? cDef.name : 'Невідомий товар')}
                  </h3>
                  ...
                  <div className="relative w-32 aspect-[2/3] mb-6 flex justify-center items-center shadow-xl rounded-xl overflow-hidden border-2 border-fuchsia-500/50">
                    <img
                      src={item.image || (cDef ? cDef.image : '')}
                      alt="item"
                      className="w-full h-full object-cover"
                    />
                    {cDef && (
...
function MarketView({
  ...
  const activeListings = marketListings.filter(
    (l) =>
      (!l.status || l.status === 'active') && (tab === 'my' ? l.sellerUid === currentUserUid : true)
  );
    ...
    <div className="pb-10 animate-in fade-in zoom-in-95 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex bg-neutral-900 border border-neutral-800 rounded-xl p-1 w-full sm:w-auto">
          <button
            onClick={() => setTab('all')}
            className={`flex-1 sm:px-6 py-2 rounded-lg font-bold text-sm transition-colors ${tab === 'all' ? 'bg-blue-600 text-white' : 'text-neutral-400 hover:text-white'}`}
          >
            Всі лоти
          </button>
          <button
            onClick={() => setTab('my')}
            className={`flex-1 sm:px-6 py-2 rounded-lg font-bold text-sm transition-colors ${tab === 'my' ? 'bg-blue-600 text-white' : 'text-neutral-400 hover:text-white'}`}
          >
            Мої продажі
          </button>
        </div>
        ...
        <div className="text-center py-20 bg-neutral-900/30 rounded-3xl border-2 border-dashed border-neutral-800">
          ...
          {tab === 'my' && (
            <p className="text-sm text-neutral-500 mt-2">
              Перейдіть в Інвентар, щоб виставити картку на продаж.
            </p>
          )}
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
          {activeListings.map((listing) => {
            ...
            return (
              <div
                key={listing.id}
                className="flex flex-col items-center animate-in zoom-in-95 group"
              >
                <div
...
function ListingModal({ listingCard, setListingCard, listOnMarket, isProcessing }) {
  ...
  return (
    <div
      ...
      onClick={() => !isProcessing && setListingCard(null)}
    >
      <div
        className="bg-neutral-900 border border-blue-900/50 p-6 rounded-3xl shadow-[0_0_50px_rgba(37,99,235,0.2)] max-w-sm w-full animate-in zoom-in-95 relative"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center gap-3 mb-6 border-b border-neutral-800 pb-4">
          <div className="w-16 h-24 rounded-lg overflow-hidden border border-neutral-700 shrink-0">
            <img
              src={listingCard.image}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                required
                className="w-full bg-neutral-950 border border-neutral-700 rounded-xl pl-12 pr-4 py-4 text-white focus:border-blue-500 outline-none text-lg font-bold"
              />
            </div>
          </div>
          <div className="flex gap-3 pt-2">
            <button
              type="submit"
              disabled={isProcessing}
              className="flex-1 bg-blue-600 hover:bg-blue-500 disabled:bg-neutral-800 disabled:text-neutral-500 text-white font-black py-4 rounded-xl transition-colors shadow-lg shadow-blue-900/20"
            >
              Продати
            </button>
            <button
              ...
              className="bg-neutral-800 hover:bg-neutral-700 text-white font-bold py-4 px-6 rounded-xl transition-colors"
            >
              Скасувати
            </button>
          </div>
...
function ShopView({
  ...
  if (pulledCards && pulledCards.length > 0) {
    const totalSellPrice = pulledCards.reduce(
      (acc, c) => acc + (c.sellPrice ? Number(c.sellPrice) : SELL_PRICE),
      0
    );
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      className={`relative w-full aspect-[2/3] rounded-xl border-2 overflow-hidden bg-neutral-900 mb-3 transition-all duration-300 group-hover:-translate-y-2 group-hover:shadow-[0_15px_30px_rgba(0,0,0,0.6)] ${style.border} ${effectClass}`}
                    >
                        ...
                        {(s.cardIds || []).slice(0, 5).map((cId, i) => {
                          ...
                          return (
                            <img
                              ...
                              src={c.image}
                              alt="m"
                              className="inline-block h-12 w-8 object-cover rounded border border-neutral-700 bg-neutral-950"
                            />
                          );
                        })}
...
function RatingView({ db, appId, currentUid, setViewingPlayerProfile }) {
  ...
  if (loading)
    return (
      <div className="text-center py-20 text-neutral-500">
        <Loader2 className="animate-spin mx-auto mb-4 w-12 h-12" /> Завантаження Залу Слави...
      </div>
    );
          ...
          return (
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`w-10 h-10 flex items-center justify-center font-black text-lg rounded-xl border transition-transform group-hover:scale-110 shrink-0 ${
                    realRank === 1
                ...
                <div className="min-w-0">
                  <div className="font-bold text-white flex items-center gap-2 text-base sm:text-lg truncate">
                    {leader.nickname}
                    <span
                      className="bg-red-900/40 text-red-400 text-xs px-2 py-0.5 rounded-lg border border-red-800 flex items-center gap-1 shrink-0"
                      title={`Рівень мисливця: ${leader.farmLevel || 1}`}
                    >
                      <Swords size={12} /> {leader.farmLevel || 1}
                    </span>
                    {leader.isBanned && (
                      <Ban size={14} className="text-red-600 shrink-0" title="Забанений" />
                    )}
                    {leader.uid === currentUid && (
                      <span className="text-[10px] bg-yellow-500/20 text-yellow-500 px-2 py-0.5 rounded-full shrink-0">
                        ВИ
                      </span>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-4 shrink-0">
                <div className="text-right">
                  ...
                  <div
                    className={`text-xl font-black ${ratingSort === 'cards' ? 'text-blue-400' : 'text-yellow-500'}`}
                  >
                ...
                <ArrowLeft
                  size={16}
                  className="text-neutral-600 group-hover:text-yellow-500 transform rotate-180 transition-colors hidden sm:block"
                />
              </div>
            </div>
          );
...
function PublicProfileView({
    ...
    const fetchPlayerData = async () => {
      try {
        const profileSnap = await getDocs(
          query(collection(db, 'artifacts', appId, 'public', 'data', 'profiles'))
        );
        ...
        if (foundProfile) {
          ...
          if (foundProfile.mainShowcaseId) {
            const showcaseDoc = await getDoc(
              doc(
                db,
                'artifacts',
                appId,
  ...
  if (loading)
    return (
      <div className="text-center py-20 text-neutral-500">
        <Loader2 className="animate-spin mx-auto w-12 h-12 mb-4" /> Завантажуємо дані гравця...
      </div>
    );
  if (!playerInfo)
    return (
  ...
  const isPlayerPremium =
    ...
    playerInfo.premiumUntil &&
    new Date(playerInfo.premiumUntil) > new Date();
  ...
  return (
    <div className="animate-in slide-in-from-right-8 duration-500">
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-8 text-center relative overflow-hidden mb-8 shadow-xl">
        <div
          className={`absolute top-0 left-0 w-full h-32 bg-gradient-to-b ${playerInfo.isBanned ? 'from-red-900/40' : playerInfo.isSuperAdmin ? 'from-orange-900/40' : playerInfo.isAdmin ? 'from-purple-900/40' : isPlayerPremium ? 'from-fuchsia-900/30' : 'from-blue-900/20'} to-transparent`}
        ></div>

        <div className="relative w-24 h-24 mx-auto mb-4 z-10">
          <PlayerAvatar
            profile={playerInfo}
            className={`w-full h-full rounded-full text-4xl ${isPlayerPremium ? 'border-4 border-fuchsia-500 shadow-[0_0_20px_rgba(217,70,239,0.5)]' : ''}`}
            iconSize={48}
          />
          {isPlayerPremium && (
            <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-neutral-900 rounded-full p-1 border-2 border-fuchsia-500 z-20">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 relative z-10 mt-6 max-w-2xl mx-auto">
          ...
          <div className="bg-neutral-950 border border-neutral-800 rounded-2xl p-4 flex flex-col items-center">
            <LayoutGrid className="text-blue-500 mb-2 w-6 h-6" />
            <span className="text-xl font-black text-white">
              {playerInfo.uniqueCardsCount || playerInventory.length}
            </span>
            <span className="text-[10px] text-neutral-500 font-bold uppercase mt-1">
              Унікальних карт
            </span>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <select
            ...
            className="bg-neutral-900 border border-neutral-700 text-sm font-medium rounded-xl px-4 py-2 w-full sm:w-auto focus:outline-none text-white cursor-pointer hover:bg-neutral-800"
          >
            {categories.map((c) => (
              <option key={c} value={c}>
                {c === 'all' ? 'Всі Категорії' : c}
          ...
          <select
            value={filterPack}
            onChange={(e) => setFilterPack(e.target.value)}
            className="bg-neutral-900 border border-neutral-700 text-sm font-medium rounded-xl px-4 py-2 w-full sm:w-auto focus:outline-none text-white cursor-pointer hover:bg-neutral-800 h-full"
          >
            <option value="all">Всі Паки</option>
            {relevantPacks.map((p) => (
              <option key={p.id} value={p.id}>
          ...
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value)}
            className="bg-neutral-900 border border-purple-900/50 text-sm font-bold rounded-xl px-4 py-2 w-full sm:w-auto focus:outline-none text-purple-400 cursor-pointer hover:bg-neutral-800"
          >
            <option value="rarity">За Рідкістю</option>
            <option value="pack">За Паком</option>
            <option value="amount">За Дублікатами</option>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
          {filteredInventory.map((item, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`relative w-full aspect-[2/3] rounded-xl border-2 overflow-hidden bg-neutral-900 mb-2 ${style.border} ${effectClass}`}
                >
                  {Number(item.card.maxSupply) > 0 && (
                    <div className="absolute top-1 left-1 bg-black/90 text-white font-black text-[9px] px-1.5 py-0.5 rounded-sm z-10 border border-neutral-700 shadow-xl">
...
function AdminView({
  ...
  useEffect(() => {
    ...
    if (activeTab === 'logs' && currentProfile.isSuperAdmin) {
      const unsub = onSnapshot(
        collection(db, 'artifacts', appId, 'public', 'data', 'adminLogs'),
        (snap) => {
          const lList = [];
  ...
  const syncAllProfiles = async () => {
    ...
    try {
      ...
      for (const userDoc of usersSnap.docs) {
        ...
        const invSnap = await getDocs(
          collection(db, 'artifacts', appId, 'users', uid, 'inventory')
        );

        let totalCardsCount = 0;
        invSnap.forEach((d) => {
          totalCardsCount += d.data().amount || 0;
        });
  ...
  const handleDeleteUser = async (userToDelete) => {
    ...
    if (userToDelete.isAdmin && !currentProfile.isSuperAdmin)
      return showToast('У вас немає прав видаляти інших адміністраторів!', 'error');
    ...
    try {
      ...
      addSystemLog(
        'Адмін',
        `Видалено акаунт гравця: ${userToDelete.nickname} (${userToDelete.uid})`
      );
    } catch (e) {
      console.error(e);
  ...
  const submitBan = async (e) => {
    ...
    try {
      await updateDoc(
        doc(db, 'artifacts', GAME_ID, 'public', 'data', 'profiles', banModalUser.uid),
        {
          isBanned: true,
      ...
      addSystemLog(
        'Адмін',
        `БАН: Гравець ${banModalUser.nickname}. Причина: ${banReason}. Термін: ${banUntil ? banUntil : 'Назавжди'}`
      );
  ...
  const toggleAdminStatus = async (userObj) => {
    ...
    try {
      ...
      addSystemLog(
        'Адмін',
        `Зміна прав: ${userObj.nickname} тепер ${newStatus ? 'Адмін' : 'Гравець'}`
      );
    } catch (e) {
      console.error(e);
  ...
  const changeUserNickname = async () => {
    ...
    try {
      ...
      profilesSnap.forEach((d) => {
        if (
          d.data().nickname?.toLowerCase() === adminNewNickname.trim().toLowerCase() &&
          d.id !== viewingUser.uid
        )
          exists = true;
      });
      ...
      addSystemLog(
        'Адмін',
        `Змінено нікнейм гравцю з ${viewingUser.nickname} на ${adminNewNickname.trim()}`
      );
  ...
  const giveCardToUser = async () => {
    ...
    try {
      ...
      addSystemLog(
        'Адмін',
        `Видано ${adminAddCardAmount} шт. картки '${cDef?.name}' гравцю ${viewingUser.nickname}`
      );
  ...
  const giveCoinsToUser = async () => {
    ...
    try {
      ...
      addSystemLog(
        'Адмін',
        `${actionText} ${Math.abs(adminAddCoinsAmount)} монет гравцю ${viewingUser.nickname}`
      );
  ...
  const setExactCoinsToUser = async () => {
    ...
    try {
      ...
      addSystemLog(
        'Адмін',
        `Встановлено точний баланс ${val} монет гравцю ${viewingUser.nickname}`
      );
  ...
  const setPlayerFarmLevel = async () => {
    ...
    try {
      ...
      await setDoc(
        farmRef,
        { bossId: null, currentHp: null, pendingCoins: 0, cooldownUntil: null },
        { merge: true }
      );
  ...
  const removeCardFromUser = async (cardId, currentAmount) => {
    ...
    try {
      ...
      addSystemLog(
        'Адмін',
        `Вилучено 1 шт. картки '${cDef?.name}' у гравця ${viewingUser.nickname}`
      );
      handleInspectUser(viewingUser.uid);
    } catch (e) {
      console.error(e);
  ...
  const handleAddBoss = async (e) => {
    ...
    let updatedBosses;
    if (newBoss.id) {
      updatedBosses = bosses
        .map((b) => (b.id === newBoss.id ? bossData : b))
  ...
  const handlePremiumAction = async (e, action) => {
    ...
    try {
      if (action === 'revoke') {
        await updateDoc(
          doc(db, 'artifacts', appId, 'public', 'data', 'profiles', premiumModalUser.uid),
          {
            isPremium: false,
            premiumUntil: null,
          }
  ...
  const globalWipe = async () => {
    ...
    if (
      !confirm(
        'УВАГА, МІЙ ЛОРД! Це повністю видалить всі інвентарі, лоти на ринку, вітрини та скине баланс і статистику всіх гравців. Продовжити?'
      )
    )
      return;
    ...
    try {
      ...
      const marketSnap = await getDocs(
        collection(db, 'artifacts', appId, 'public', 'data', 'market')
      );
      marketSnap.forEach((d) => deleteDoc(d.ref));

      // 2. Очищаємо лічильники ліміток (якщо вони там вже створилися)
      const statsSnap = await getDocs(
        collection(db, 'artifacts', appId, 'public', 'data', 'cardStats')
      );
      ...
      for (const profileDoc of profilesSnap.docs) {
        ...
        const showcasesSnap = await getDocs(
          collection(db, 'artifacts', appId, 'users', uid, 'showcases')
        );
      ...
      addSystemLog(
        'Адмін',
        'ГЛОБАЛЬНИЙ ВАЙП: інвентарі, ринок, статистика та баланс скинуті до нуля.'
      );
    } catch (error) {
      console.error(error);
    ...
    <div className="max-w-4xl mx-auto pb-10 relative">
      ...
      <div className="flex gap-2 mb-6 bg-neutral-900 p-2 rounded-xl overflow-x-auto hide-scrollbar">
        <button
          ...
          className={`flex-1 min-w-[100px] py-3 rounded-lg font-bold flex justify-center items-center gap-2 ${activeTab === 'users' ? 'bg-purple-600 text-white' : 'text-neutral-400 hover:bg-neutral-800'}`}
        >
          <Users size={18} /> Гравці
        </button>
        <button
          onClick={() => setActiveTab('packs')}
          className={`flex-1 min-w-[100px] py-3 rounded-lg font-bold flex justify-center items-center gap-2 ${activeTab === 'packs' ? 'bg-purple-600 text-white' : 'text-neutral-400 hover:bg-neutral-800'}`}
        >
          <Layers size={18} /> Паки
        </button>
        <button
          onClick={() => setActiveTab('cards')}
          className={`flex-1 min-w-[100px] py-3 rounded-lg font-bold flex justify-center items-center gap-2 ${activeTab === 'cards' ? 'bg-purple-600 text-white' : 'text-neutral-400 hover:bg-neutral-800'}`}
        >
        ...
        {currentProfile.isAdmin && (
          <>
            <button
              ...
              className={`whitespace-nowrap flex items-center gap-2 px-4 py-2 sm:py-3 rounded-xl font-bold transition-all text-sm sm:text-base ${activeTab === 'bosses' ? 'bg-red-600/20 text-red-500 border border-red-500/50' : 'bg-neutral-900 border border-neutral-800 text-neutral-400 hover:text-white'}`}
            >
              <Swords size={18} /> Боси
            </button>
            <button
              onClick={() => setActiveTab('promos')}
              className={`flex-1 min-w-[100px] py-3 rounded-lg font-bold flex justify-center items-center gap-2 ${activeTab === 'promos' ? 'bg-purple-600 text-white' : 'text-neutral-400 hover:bg-neutral-800'}`}
            >
              <Ticket size={18} /> Коди
            </button>
            <button
              onClick={() => setActiveTab('premiumShop')}
              className={`flex-1 min-w-[100px] py-3 rounded-lg font-bold flex justify-center items-center gap-2 ${activeTab === 'premiumShop' ? 'bg-fuchsia-600 text-white' : 'text-fuchsia-400/70 hover:bg-neutral-800'}`}
            >
              <Gem size={18} /> Прем Товари
            </button>
            <button
              onClick={() => setActiveTab('settings')}
              className={`flex-1 min-w-[100px] py-3 rounded-lg font-bold flex justify-center items-center gap-2 ${activeTab === 'settings' ? 'bg-purple-600 text-white' : 'text-neutral-400 hover:bg-neutral-800'}`}
            >
              <Settings size={18} /> Налаштування
            </button>
            <button
              onClick={() => setActiveTab('logs')}
              className={`flex-1 min-w-[100px] py-3 rounded-lg font-bold flex justify-center items-center gap-2 ${activeTab === 'logs' ? 'bg-red-900/80 text-white border-red-500 border' : 'text-red-400 hover:bg-neutral-800'}`}
            >
              ...
              <div className="flex flex-col md:flex-row gap-4 mb-6">
                ...
                <div className="bg-neutral-950 p-4 rounded-xl border border-neutral-800 flex-1 flex flex-col gap-3 justify-end">
                  ...
                  <button
                    ...
                    className="bg-red-600 hover:bg-red-500 text-white font-bold px-4 py-2 rounded-lg w-full transition-colors h-10"
                  >
                    Встановити рівень
                  </button>
                </div>
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          >
                            ...
                            {u.isBanned && (
                              <span className="text-[10px] bg-red-900/50 text-red-400 px-2 py-0.5 rounded border border-red-800 uppercase font-black tracking-widest shrink-0">
                                Бан
                              </span>
                            )}
                          </div>
                ...
                return (
                  ...
                  >
                    <div className="w-20 aspect-[2/3] rounded-lg border border-neutral-700 overflow-hidden flex-shrink-0 relative">
                      {bCard && (
                        <img src={bCard.image} alt="boss" className="w-full h-full object-cover" />
                      )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {rarities.map((r) => (
                    <div key={r.name} className="flex flex-col">
                      <label className={`text-xs font-bold mb-1 ${COLOR_PRESETS[r.color]?.text}`}>
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              ...
              <select
                ...
                className="bg-neutral-950 border border-neutral-700 rounded-xl px-4 py-3 text-white md:col-span-2 text-purple-400 font-bold"
              >
                ...
                >
                  ...
                  <div className="p-2 text-center flex flex-col items-center flex-1 justify-between">
                    <div className="w-full">
                      <div className={`text-[10px] font-black uppercase ${style.text}`}>
                        {card.rarity}
                      </div>
